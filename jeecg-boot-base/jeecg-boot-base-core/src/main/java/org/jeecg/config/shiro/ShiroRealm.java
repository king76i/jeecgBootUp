...
public class ShiroRealm extends AuthorizingRealm {
    ...
    @Lazy
    @Resource
    private HelloUserApi helloUserApi;
    /**
     * 必须重写此方法，不然Shiro会报错
     */
    @Override
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        if (token == null) {
            log.info("token:"+token+"————————身份认证失败——————————IP地址:  "+ oConvertUtils.getIpAddrByRequest(SpringContextUtils.getHttpServletRequest()));
            throw new AuthenticationException("token为空!");
        }
        //log.info("++"+this.checkUserTokenIsEffect(token).getClass().toString()+"++");
        Object obj = this.checkUserTokenIsEffect(token);
        if(obj.getClass().toString().equals(UserInfo.class.toString())){
            // 校验token有效性
    ...
    public Object checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        log.info(username+":"+type);
        if(type!=null&&type.equals("api")){
            if (username == null) {//此处username是userId
                throw new AuthenticationException("token非法无效!");
            }
            // 查询用户信息
            log.debug("———校验token是否有效————checkUserTokenIsEffect——————— "+ token);

            UserInfo info = helloUserApi.getOnes(username);

            if (info == null) {
                throw new AuthenticationException("用户不存在!");
            }

            if (!jwtTokenRefresh(token, username, "123456")) {
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        ...
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            ...
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                ...
                redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME * expireTime);
