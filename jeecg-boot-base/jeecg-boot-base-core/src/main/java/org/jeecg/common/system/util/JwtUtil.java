...
public class JwtUtil {
	public static void main(String[] args) {
		String sign = sign("admin","123456");
		String token = JwtUtil.signVer2("1508620929936101377", "123456");
		// 设置token缓存有效时间
	 ...
	 * @return 加密的token
	 */
	public static String signVer2(String username, String secret) {
		Date date = new Date(System.currentTimeMillis() + (EXPIRE_TIME * 84));
		Algorithm algorithm = Algorithm.HMAC256(secret);
		// 附带username信息
		return JWT.create().withClaim("username", username).withClaim("type","api").withExpiresAt(date).sign(algorithm);
